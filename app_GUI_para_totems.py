...
def generar_info_qr(nombre_pelicula:str,ubicacion_totem:str,cantidad_entradas:int)->str:
    ...
    return (" ").join([nombre_pelicula,ubicacion_totem,str(cantidad_entradas),timestamp_compra])
...
def finalizar_operacion(nombre_pelicula,ubicacion_totem,cantidad_entradas):

    id_codigo = generar_info_qr(nombre_pelicula,ubicacion_totem,cantidad_entradas)

    generar_pdf_código_e_id_qr(id_codigo)
...
def Checkout(cant_tickets,location,list_snack,snacks,nombre_pelicula) -> None:
    ...
    pagar = lambda: finalizar_operacion(nombre_pelicula,location,cant_tickets)
...
def Pantalla_Reserva(dato_cine, id_pelicula) -> None:
    ...
    checkout = lambda: Checkout(cant_entradas[0],location,snacks_adquiridos,snacks,nombre_pelicula)
